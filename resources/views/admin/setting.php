...
// Admin settings page - for site configuration, user management, etc.
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Handle settings update logic here
    // For example, save settings to database or config file
    // This is just a placeholder for demonstration
    $message = "Settings updated successfully!";
}
//settings.php is only for admin, teachers should not access it, they will have their own dashboard with quiz management features
if ($userRole === 'teacher' && isset($_GET['page']) && $_GET['page'] === 'settings') {
    $_SESSION['error'] = "Unauthorized access!";
    header('Location: settings.php');
    exit();
}

//make it can connect with dashboard.php, when admin click settings in dashboard, it will redirect to this page, and when teacher click settings in their dashboard, it will redirect to their own settings page, which is not implemented yet, but will be in the future
$userRole = isset($_SESSION['role']) ? $_SESSION['role'] : '';
if ($userRole === 'admin' && isset($_GET['page']) && $_GET['page'] === 'settings') {
    // Admin settings page
    header('Location: settings.php');
    exit();
} elseif ($userRole === 'teacher' && isset($_GET['page']) && $_GET['page'] === 'settings') {
    // Teacher settings page (not implemented yet)
    header('Location: teacher_settings.php');
    exit();
} else {
    // Default to admin settings if no specific page is requested
    header('Location: settings.php');
    exit();
}
?>
    ...
    <div class="settings-container">
        ...
        <div class="settings-card">
            ...
            <form method="POST" action="/admin/settings/users">
                ...
                <div class="mb-3">
                    <label for="userRegistration" class="form-label">Allow User Registration</label>
                    <div class="form-check form-switch">
                ...
                <div class="mb-3">
                    <label for="emailVerification" class="form-label">Email Verification Required</label>
                    <div class="form-check form-switch">
                ...
                <button type="submit" class="btn btn-primary"><i class="fas fa-save"></i> Save User Settings</button>
            </form>
        </div>
    </div>
